...
class TestSlr(unittest.TestCase):
    def test_st(self):
        ...
        for idx, filt in enumerate(filts):
            pulse = slr.dzrf(N, tb, ptype="st", ftype=filt, d1=0.01, d2=0.01)
    ...
    def test_inv(self):
        ...
        for idx, filt in enumerate(filts):
            pulse = slr.dzrf(N, tb, ptype, filt, d1, d2)

            [_, b] = sim.abrm(pulse, np.arange(-2 * tb, 2 * tb, 0.01))
            mz = 1 - 2 * np.abs(b) ** 2
    ...
    def test_root_flipped(self):
        ...
        b = bsf * slr.dzmp(N, tb, d1, d2)
        b = b[::-1]
        [pulse, _] = slr.root_flip(b, d1, flip, tb, verbose=False)
    ...
    def test_recursive(self):
        ...
        [pulses, _] = slr.dz_recursive_rf(nseg, tb, n, se_seq, tb_ref)

        mz = np.ones(np.size(np.arange(-4 * tb, 4 * tb, 0.01)))
        for ii in range(0, nseg):
            [a, b] = sim.abrm(pulses[:, ii], np.arange(-4 * tb, 4 * tb, 0.01), True)
